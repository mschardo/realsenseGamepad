...
uint16_t* filterData(uint16_t* data, int width, int height, uint16_t max, uint16_t min) {
...
int main(int argc, char** argv)
{
    ...
    {
      cv::Mat frame(cv::Size(depth_intrin.width,depth_intrin.height), CV_16UC1, filterData((uint16_t*)f.get_data(),depth_intrin.width,depth_intrin.height,750,500));
    ...
    while (1) {
      ...
      if (frames == 6) {
        ...
        axisHistory[frames++] = getAxis(rgb, 30);
      }
      cv::waitKey(1);
    }
    dev->stop();
